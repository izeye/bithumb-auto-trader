...
public class DefaultBithumbApiService implements BithumbApiService {
    ...
    public DefaultBithumbApiService(BithumbProperties properties, BithumbMacService bithumbMacService, WebClient.Builder webClientBuilder) {
    ...
    public String tradePlace(TradePlaceRequest request) {
        ...
        Map<String, Object> responseMap = this.webClient.post()
                .uri(URI_TRADE_PLACE)
                .header("Api-Key", this.properties.getConnectKey())
                .header("Api-Sign", mac)
                .header("Api-Nonce", String.valueOf(nonce))
                .header("api-client-type", "2")
                .contentType(MediaType.APPLICATION_FORM_URLENCODED)
                .body(BodyInserters.fromFormData(parameters))
                .exchangeToMono((response) -> response.bodyToMono(MAP_STRING_OBJECT))
                .block();
